...
describe('handle output based on entering type', () => {
    describe('works', () => {
        ...
        it.each(cases)('The enum should include the output of the method', input => expect(values.includes(StepGenerator.handleOutputForUi(input) as OutputForUiEnum)).toBe(true))
    })
    describe('does not work', () => {
        it('Should return false when value does not return an enum instance', () => expect(values.includes('Test' as OutputForUiEnum)).toBe(false))
    })
})
